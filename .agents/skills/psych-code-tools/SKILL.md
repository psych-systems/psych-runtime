---
name: psych-code-tools
description: >-
  Register Python functions as Psych tools with `ToolRegistry.register` and
  `register_dynamic`: schema derivation from type hints, descriptions from
  docstrings, the `interruptible` / `safe_to_retry` / `annotations` flags, and
  how a registered name reaches an AgentSpec. Use whenever someone gives a Psych
  agent a Python function to call, hits `ToolSchemaError`, asks why a tool is
  missing from the model's tool list, needs tools whose shape is only known at
  runtime (from a database of integrations or a customer-authored form), or asks
  which flag stops a refund being half-issued on abort. Read before writing any
  `@registry.register`, because `interruptible=False` and `safe_to_retry` default
  to the safe answer and getting them wrong causes duplicate side effects.
---
