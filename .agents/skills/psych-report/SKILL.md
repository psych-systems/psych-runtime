...
report.steps  # StepReport, workflow only: one row per attempt, with path, parent_step_id,
              # iteration, skipped, will_retry, replayed_from, started_at, completed_at
...
A `ToolCallReport` also carries `attachments`: the named outputs kept beside a
result, which is how a code execution stdout, stderr, returned value and files
reach a projection. Each names its content type, its size, whether it was
truncated, and whether it is still readable (`inline`, `blob`, or
`preview_only`). A console renders the preview and reads the rest through
`read_tool_output` by window or pattern rather than inlining it. See
`psych-blobs` and `psych-sandbox`.
...
## workflow_view(): the step tree

```python
view = await psych_runtime.workflow_view(store, run_id, scope=scope)
view.steps            # StepView per top-level step, in Spec order, children nested
view.waiting          # the step the Run is parked on, and why
view.state, view.output, view.step_starts, view.completed, view.failed
view.replays_run_id, view.replay_from_step, view.breakpoints, view.step_mode
```

Each `StepView` carries `status` (`pending running waiting completed failed
retrying skipped replayed`), `attempts`, `input`, `output`, `failure`,
`started_at`, `completed_at`, `retry_at`, `iteration`, `children` and, for a
branch, `cases`. Steps the Run has not reached yet are present as `pending`,
from the Spec; loop iterations and foreach items are read from the log, by
parent. Raises `WorkflowRequired` for an agent Run. See `psych-workflows`.
