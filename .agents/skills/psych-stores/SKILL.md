...
`MySQLStore` sets `READ COMMITTED` as the pool's `init_command`, so every
connection the pool opens lazily has it; a pool you pass in must do the same or
`claim()` and `expired_leases()` take gap locks under REPEATABLE READ and turn
independent Workers into a queue. The MySQL schema has no `CHECK` on `state`
on purpose: some servers parse and ignore one, so the adapter and the reducer
are what refuse a state outside the enum.

`DynamoDBStore.create_run` with an idempotency key writes the key and the
header in one `TransactWriteItems`, so a crash between the two cannot strand a
key whose Run has no header. Everything else is one conditional write per item.
...
- **`FilesystemBlobStore` writes are atomic.** Content and its metadata sidecar
  each land through a temporary file and a rename, content first, so a reader
  on a shared filesystem never sees a partial result. See `psych-blobs`.
