...
Approvals go through the door too. A `call_tool` invocation is gated on the
**target** tool's own name and annotations, exactly as if its catalogue had
been preloaded: a `@destructive` selector that suspends `github__delete_repo`
when preloaded suspends the same call made through `call_tool`, and a `Policy`
is asked about `github__delete_repo`, never about `call_tool`. A target the Run
...
`McpServerUnreachable.reason` is always a sentence worth showing: a transport
error with no text of its own (an `asyncio.TimeoutError`, a bare `OSError`)
reads "the connection timed out" or names the exception type, and the message
carries no trailing colon when there is nothing to put after it. A console can
put `str(error)` in the row of the connection that failed as it is.

## A catalogue too large to accept

A Streamable HTTP server writes each response as one server-sent event, and an
event has a size ceiling, so the ceiling applies to `tools/list` — which for a
server generated from an OpenAPI description can run to several megabytes.

Exceeding it raises `McpResponseTooLarge`, **not** `McpServerUnreachable`: the
server answered correctly and in full, and this client declined to buffer the
reply. The distinction is the point. Were it unreachable, `optional=True` would
drop a healthy server and the agent would run without its tools with nobody
told why.

Raise the ceiling with `psych_runtime.set_max_sse_event_bytes`. It is process-wide,
because it belongs to the HTTP client every connection shares. Leave it
bounded: an MCP server is a third party, and a reply nothing bounds can exhaust
...
## MCP tools inside `run_code`

An agent with `code_execution` can call **every MCP tool it is currently
granted** from inside a program, through the same pool, the same connection,
the same OAuth and the same three narrowing planes. There is no second MCP
client, no MCP credential inside the sandbox, and no separate configuration
surface: a program reaches the agent's existing connections or nothing.

```python
rows = []
for sku in ("A", "B", "C"):
    rows.append(await support__lookup(sku=sku))  # identifier names
raw = await call_tool("docs__list-repos", {"owner": "psych"})  # any name
```

Three things follow from "the same narrowing", and each is worth knowing:

- **A deferred server's tools are bindable.** They are absent from the prompt
  and present in the program, which is the case code execution is best at: a
  program pays nothing for a schema it never reads. `list_tools` and
  `get_tool_info` are callable from a program too, for the argument shapes.
- **Server-qualified names stay qualified.** Two servers both offering `search`
  remain independently callable as `docs__search` and `tickets__search`, and
  each routes to its own scoped connection.
- **The set is re-resolved every turn.** A tool the server removed, or a tenant
  policy withdrew, stops being callable from a program in the same turn it
  stops being callable directly. A Spec that named it explicitly gets it back
  as `unavailable_bindings` in the `run_code` result, never as a substitute.

A binding call is recorded as its own `tool_call_started`/`tool_call_finished`
pair carrying `parent_call_id`, so a report shows forty MCP calls nested under
the one `run_code` that made them.

MCP failures keep their identity inside the program: `ToolError.kind` is
`McpToolError` when the tool ran and said no, and `McpServerUnreachable` when
it never ran. A server asking for elicitation arrives as
`input_required_in_program`, because nothing can answer it while a subprocess
waits — the model is told to make that call itself.
...
- **Unannotated MCP tools are treated as `write` for approvals,** and a
  ...
  `psych-approvals`.
- **A static tool may not wear a server's prefix.** A `CodeTool` named
  `github__search_issues` beside a server named `github` is refused at
  publish; otherwise the two collide at run time once the server exposes
  `search_issues`.
- **The bounded SSE event source is installed on every connection,** not only
  at import, so an SDK module imported lazily cannot keep the SDK's 1 MB
  default.
