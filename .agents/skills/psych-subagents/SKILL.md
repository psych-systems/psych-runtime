...
suspend at all if they have already finished. Each child that finishes wakes a
suspended parent once: a parent woken while a sibling is still running gets a
turn, and suspends again if it has nothing else to do. A scripted model for a
parent therefore has to answer from the conversation rather than count turns.
