...
- `process_count` includes the sandbox interpreter and processes it creates.
  On POSIX, Psych adds the target user's existing process/thread count before
  applying `RLIMIT_NPROC`, because the kernel accepts an absolute per-user
  ceiling rather than an execution-local count. Runs sharing a user still
  share that kernel counter; use a distinct `run_as` uid per worker when the
  ceiling must be independent across concurrent executions.
