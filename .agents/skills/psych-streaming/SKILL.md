...
`subscribe(store, run_id, on_record)` returns a task that pumps records into a
callback, for a websocket or SSE bridge. A task nobody awaits would otherwise
report its exception at garbage collection, if ever, so a pump that dies — a
`CorruptLog`, a raising callback — is logged under `psych.runtime.stream` the
moment it happens. Keep a reference to the task and check `task.exception()`
